...
def generate_samples_grid(autoencoder, diffusion, n_per_class=5, save_dir="./results"):
    ...
    for i in range(n_classes):
        ...
        for j in range(n_per_class):
            img = decoded_cpu[j].permute(1, 2, 0).numpy()  # Change from [C,H,W] to [H,W,C] for plotting
...
def train_conditional_diffusion(autoencoder, unet, train_loader, num_epochs=100, lr=1e-3, visualize_every=10,
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        current_res_scale = compute_res_scale(epoch, start_epoch + num_epochs, initial=2, final=2.5)
        epoch_loss = 0
...
def main(checkpoint_path=None, total_epochs=2000):
    ...
    if 'diffusion' not in locals():
        ...
        conditional_unet, diffusion, diff_losses = train_conditional_diffusion(
            autoencoder, conditional_unet, train_loader, num_epochs=remaining_epochs, lr=1e-5,
            visualize_every=20,
            save_dir=results_dir,
            device=device,
...
if __name__ == "__main__":
    main(checkpoint_path='cifar_cat_dog_conditional_improved/conditional_diffusion_epoch_2350.pt', total_epochs=10000)

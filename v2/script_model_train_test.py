...
class Encoder(nn.Module):
    def __init__(self, in_channels=3):
        ...
        # Class-specific features pathway
        ...
        self.recon_branch = nn.Sequential(
            nn.Conv2d(128, 64, 3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 2, 1)  # 2 channels for reconstruction
        )

    def forward(self, x):
        features = self.backbone(x)
        class_features = self.class_branch(features)
        recon_features = self.recon_branch(features)

        # Combine for 3-channel output
        return torch.cat([class_features, recon_features], dim=1)


# Decoder network with CAB blocks for RGB latent space
class Decoder(nn.Module):
    def __init__(self, out_channels=3):
        super().__init__()

        # Decoder starts directly from the 3-channel 8x8 latent
        self.decoder = nn.Sequential(
            # Initial expansion from 3 to 128 channels
    ...
    def forward(self, z):
        # z is already in shape [batch_size, 3, 8, 8]
...
# Enhanced autoencoder with classifier and center loss
class SimpleAutoencoder(nn.Module):
    def __init__(self, in_channels=3, latent_dim=None, num_classes=2):
        super().__init__()
        # Keep original encoder and decoder unchanged
        self.encoder = Encoder(in_channels)
        self.decoder = Decoder(in_channels)

        # Latent dimension remains unchanged for backward compatibility
        self.latent_dim = 3 * 8 * 8

        # Add classifier and center loss
        # Replace the current classifier with a more powerful one
    ...
    def compute_center_loss(self, z_flat, labels):
        # New method to compute center loss
        return self.center_loss(z_flat, labels)

    def forward(self, x):
        # Keep original forward pass unchanged for backward compatibility
        z = self.encode(x)
...
def main():
    ...
    results_dir = "./cifar_cat_dog_conditional_v2"
    ...
    if os.path.exists(autoencoder_path):
        ...
        def train_autoencoder(autoencoder, train_loader, num_epochs=100, lr=1e-4, lambda_cls=5.0, lambda_center=2.0,
            ...
            """Train autoencoder with separate optimization paths to avoid in-place operation conflicts"""
            print("Starting Autoencoder training with class separation...")
            os.makedirs(save_dir, exist_ok=True)
            device = next(autoencoder.parameters()).device
            ...
            loss_history = {'total': [], 'recon': [], 'class': [], 'center': []}
            ...
            for epoch in range(num_epochs):
                ...
                for batch_idx, (data, labels) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}")):
                    ...
                    # APPROACH 1: Separate forward passes for each optimization task

                    # Step 1: Reconstruction optimization
                    # This path updates encoder + decoder only
                    recon_optimizer.zero_grad()
                    z_recon = autoencoder.encode(data)
                    reconstructed = autoencoder.decode(z_recon)
                    recon_loss = euclidean_distance_loss(reconstructed, data)
                    recon_loss.backward()  # No retain_graph needed
                    recon_optimizer.step()

                    # Step 2: Classification optimization
                    # This path uses a separate forward pass and only updates classifier + center loss
                    class_optimizer.zero_grad()
                    # Detach encoder output to prevent gradients flowing back into encoder
                    with torch.no_grad():
                        z_class = autoencoder.encode(data)
                    z_flat = z_class.view(z_class.size(0), -1).detach()  # Detach to prevent gradient flow

                    # Forward through classification branch only
                    class_logits = autoencoder.classify(z_flat)
                    class_loss = F.cross_entropy(class_logits, labels)
                    center_loss = autoencoder.compute_center_loss(z_flat, labels)

                    # Combined classification loss
                    total_class_loss = lambda_cls * class_loss + lambda_center * center_loss
                    total_class_loss.backward()  # No retain_graph needed

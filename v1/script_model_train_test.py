...
class Encoder(nn.Module):
    def __init__(self, in_channels=3):
        ...
        self.encoder = nn.Sequential(
            ...
            CAB(16, reduction=8),
            nn.ReLU(inplace=True),
            nn.Conv2d(16, 32, 4, stride=2, padding=1),  # 32x32 -> 16x16, 16 -> 32 channels

            # Second stage: 32 -> 64 channels
            nn.Conv2d(32, 64, 3, stride=1, padding=1),
            CAB(64, reduction=8),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 128, 4, stride=2, padding=1),  # 16x16 -> 8x8

            # Final stage: project to 3 channels for RGB latent
            nn.Conv2d(128, 3, 3, stride=1, padding=1),
            nn.ReLU(inplace=True)
        )

    def forward(self, x):
        # Returns [batch_size, 3, 8, 8] tensor
...
class SimpleAutoencoder(nn.Module):
    def __init__(self, in_channels=3, latent_dim=None):
        super().__init__()
        self.encoder = Encoder(in_channels)
        self.decoder = Decoder(in_channels)
        # Latent dimension is fixed at 3x8x8=192 per sample
        self.latent_dim = 3 * 8 * 8
        if latent_dim is not None:
            print(f"Warning: latent_dim={latent_dim} parameter is ignored in RGB latent space model")

    def encode(self, x):
        # Returns latent in shape [batch_size, 3, 8, 8]
        return self.encoder(x)

    def decode(self, z):
        # Takes latent in shape [batch_size, 3, 8, 8]
        # If z is flattened, reshape it
        if len(z.shape) == 2:
            # Assume flattened batch of latents
...
def main():
    ...
    results_dir = "./cifar_cat_dog_conditional"
    ...
    if os.path.exists(autoencoder_path):
        ...
        def train_autoencoder(autoencoder, num_epochs=100, lr=1e-4, visualize_every=1, save_dir=results_dir):
        ...
        autoencoder, ae_losses = train_autoencoder(
            autoencoder, num_epochs=100, lr=1e-4,

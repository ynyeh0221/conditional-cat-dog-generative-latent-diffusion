...
def main():
    ...
    results_dir = "./cifar_cat_dog_conditional_v3"
    ...
    if os.path.exists(vae_path):
        ...
        def train_vae(vae, train_loader, num_epochs=100, lr=1e-4, lambda_cls=8.0, lambda_center=4.0,
                      lambda_kl=10.0, visualize_every=1, save_dir="./results"):
            """Train VAE with separate optimization paths to avoid in-place operation conflicts"""
            print("Starting integrated VAE training...")
            os.makedirs(save_dir, exist_ok=True)
            device = next(vae.parameters()).device
            ...
            for epoch in range(num_epochs):
                ...
                for batch_idx, (data, labels) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}")):
                    ...
                    total_loss = recon_loss + lambda_kl * kl_loss + lambda_cls * class_loss + lambda_center * center_loss

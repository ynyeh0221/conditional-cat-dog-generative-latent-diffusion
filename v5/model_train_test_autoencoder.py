...
# Redesigned Encoder with flat latent space
...
# Redesigned Decoder with flat latent space input
class Decoder(nn.Module):
    def __init__(self, latent_dim=128, out_channels=3):
        ...
        # Decoder network from spatial features to image
...
# Modified SimpleAutoencoder with flat latent space
class SimpleAutoencoder(nn.Module):
    def __init__(self, in_channels=3, latent_dim=128, num_classes=2):
        super().__init__()
        self.latent_dim = latent_dim

        # Updated encoder and decoder with flat latent space
...
# Properly modified function to generate samples grid
def generate_samples_grid(autoencoder, diffusion, n_per_class=5, save_dir="./results"):
    ...
    for i in range(n_classes):
        ...
        # Use flat latent shape instead of spatial shape
...
# Modified visualize_denoising_steps for flat latent space
...
# Properly modified function to visualize latent space
def visualize_latent_space(autoencoder, epoch, save_dir="./results"):
    ...
    with torch.no_grad():
        for images, labels in test_loader:
            images = images.to(device)
            # Get flat latent vectors directly from encoder
            latents = autoencoder.encode(images)
...
# Modified function to generate class samples with flat latent space
...
def main():
    ...
    results_dir = "./cifar_cat_dog_conditional_v5"
